...
* Problem 3
 ...
 NPU(Neural Network Processing Unit), is a processor that specializes in the
 operations over machine learning tasks. Overall, it will be more perfomant than
 a CPU and more power efficient and performant than the CPU and GPU combination.
 To achieve this, it has two major difference compared to a CPU. Firstly, it
 supports way less instructions compare to a CPU. But for the instructions it
 supports, its architecture makes usre its really efficient powerful at it. For
 example, it is really good at multiply-accumulate calculation which is used a
 lot in machine learning tasks. Furthermore, it easily supports paralized computation Computation logic is very
 simple? Good for massive multi-media data Focus on low precission operations

 Has data storage and processing together. Therefore, it will be very power
 efficient compare to the CPU and GPU collaboration

 According to this [[https://en.wikichip.org/wiki/neural_processor][Wiki Chip page]], the TPU is a specialized type of Neural
 Network Processing Unit. It is good at deep learning tasks where the NPU is
 good at machine learning tasks in general. They are also proprietary. Google
 develops and only provides it on GCP. According to [[https://www.bizety.com/2023/01/03/ai-chips-npu-vs-tpu/][this page,]] TPU is highly
 parallizable and programmable

 FPGA(Field-Programmable Gate Array), like the name suggests, it is programmable
 and could be used for prototyping etc. Via Programmable interconnect and
 programmable logic block (See [[https://www.latticesemi.com/en/What-is-an-FPGA][first figure here]]) From the hetrogenious
 comp[osition of the structure, we know it is very good at parallel computation.
 Not only in the sense of multi-chip collaboration but also in the sense of ]
** Sources
